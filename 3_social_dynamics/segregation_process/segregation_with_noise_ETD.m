% Jingbang code
% also record the segregation process and save
close all
clear
tic
...
steps = 1e15;
% rho0 = 0.8;
eps = 0.0002; % noise amplitude
everyData = 1; % how many time steps before checking for a rare event (checking costs time)
everyPlot = 1e5;  % hoe many time steps before plotting the shape
steps_record_seg = 1e6; % how many time steps used for recording
seg_process = zeros(Nx,steps_record_seg); % record segregation process
...
Nevents = 1;  % number of events required
...
for step = 1:steps
    ...
    record_step_now = mod(step-1,steps_record_seg)+1;
    seg_process(:,record_step_now) = rhoR;
...
%         pause
    ...
    if mod(step,everyData)==0
        if any(rhoR<rhoRmin)                  % any event?
            ...
            seg_process(:,record_step_now) = rhoR_int;
            seg_process = circshift(seg_process,-idx+Nx/2+1,1);
            seg_process = circshift(seg_process,steps_record_seg-record_step_now,2);
...
close
save_interval = 40;
time_to_save = linspace(step-steps_record_seg+save_interval,...
    step,steps_record_seg/save_interval)*dt;
seg_process = cat(1,time_to_save,seg_process(:,save_interval:save_interval:end));

figure(3)
plot(seg_process(2:end,:))

writematrix(seg_process,"SEG_process.txt",'Delimiter','space')
