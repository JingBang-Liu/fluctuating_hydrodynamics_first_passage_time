% try to filter energy from noise
% and also capture the final rupture process
...
rupture_process = zeros(Nx,steps_record_rupture);
x = linspace(0,L-L/Nx,Nx);
dx = x(2)-x(1);
dk = 2*pi/L;
k = [0:Nx/2,-Nx/2+1:-1]*dk;

Nevents = 10;  % number of events required
...
everyPlot = 1e4;  % how many time steps before plotting the shape
...
for step = 1:steps
  ...
  % record the process of rupture
  record_step_now = mod(step-1,steps_record_rupture)+1;
  rupture_process(:,record_step_now) = hR(1,:);

  %plotting
  if mod(step, everyPlot) == 0
	clf; plot(x, ifft(h,[],2, 'symmetric')); drawnow
  end
  
  % checking for rare events
  if mod(step,everyData)==0           
    %varhR(step)=var(hR-h0);
    ...
    if any(any(hR<z))                    % any event?
	  for c = 1:Ncopies                  % run through realisations
       ...
       %  idx=idxs(randi(size(idxs,2)));       % if there is more than one point satisfying the rare event critetion choose at random             
        ...
        rupture_process(:,record_step_now) = hR_int(c,:);
        rupture_process = circshift(rupture_process,-idx+Nx/2+1,1);
        rupture_process = circshift(rupture_process,steps_record_rupture-record_step_now,2);

		events = [events; circshift(hR_int(c,:), -idx+Nx/2+1)];  % store the event with lowest point at x=L/2
    filename = "rp_process_h1.01_seed_"+num2str(temp_seed)+"_rpcount_"+num2str(size(events,1))+".mat";
    save(filename,"rupture_process")
    rupture_process = zeros(Nx,steps_record_rupture);
		    if(startup==0)
